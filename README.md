...
사전은 색면 없이 **얇은 실선 두 줄**로만 나뉜다. 실선도 글도 벽에서 16dp 물러난다.
위에서부터
...
**판은 목록과 함께 여닫힌다.** 목록을 내리면 판이 줄고, 訓音 자리가 안 나올 만큼
줄면 손을 뗀 순간 입력 칸 한 줄로 접힌다. 다시 무언가 넣으면 펼쳐진다. 거꾸로 올리면
정사각형까지 자란 뒤 목록이 따라 온다. 사이의 손잡이로 직접 여닫아도 된다.
